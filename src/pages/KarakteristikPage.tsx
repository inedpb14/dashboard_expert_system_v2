...
import { KARAKTERISTIK_DATA } from "../../constants";
import type { Karakteristik } from "../../types";
import { PencilIcon, TrashIcon, PlusIcon } from "../../constants";

const KarakteristikPage: React.FC = () => {
    const [items, setItems] = useState<Karakteristik[]>(KARAKTERISTIK_DATA);
    const [searchTerm, setSearchTerm] = useState('');
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [currentItem, setCurrentItem] = useState<Karakteristik | null>(null);
    ...
    const handleOpenModal = (item: Karakteristik | null = null) => {
        setCurrentItem(item);
        setIsModalOpen(true);
    };
    ...
    const handleSave = (itemData: Omit<Karakteristik, 'id'>) => {
        if (currentItem) {
            setItems(items.map(i => i.id === currentItem.id ? { ...i, ...itemData } : i));
        } else {
            const newItem: Karakteristik = { id: Date.now().toString(), ...itemData };
    ...
    return (
        <div className="container mx-auto">
            <PageHeader title="Kelola Karakteristik" onButtonClick={() => handleOpenModal()} buttonIcon={<PlusIcon className="w-5 h-5 mr-2"/>} buttonText="Tambah Baru" />
                    ...
                    <table className="w-full text-sm text-left text-slate-500">
                        <thead className="text-xs text-slate-700 uppercase bg-slate-50">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3">Nama Karakteristik</th>
            ...
            {isModalOpen && (
                <Modal title={currentItem ? 'Edit Karakteristik' : 'Tambah Karakteristik'} onClose={handleCloseModal}>
                    <KarakteristikForm currentItem={currentItem} onSave={handleSave} onClose={handleCloseModal} />
                </Modal>
            )}
        </div>
    );
...
const KarakteristikForm: React.FC<{currentItem: Karakteristik | null, onSave: (data: any) => void, onClose: () => void}> = ({ currentItem, onSave, onClose }) => {
    ...
    return (
        <form onSubmit={handleSubmit}>
            <div className="mb-4">
             ...
             <div className="mb-6">
                <label htmlFor="name" className="block text-sm font-medium text-slate-700 mb-1">Nama Karakteristik</label>
...
export default KarakteristikPage;
